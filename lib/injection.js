...
function normalizeKeys(keys, name, dynamicKeys) {
  if ('string' === typeof keys) return [keys];else if (Array.isArray(keys)) return keys;else if ('function' === typeof dynamicKeys) return [];else throw new TypeError('"' + name + '" only accepts string or array of strings as second parameter when static property \'keys\' of component not exists.');
}
