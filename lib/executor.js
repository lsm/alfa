...
function executePipe(err, args, store, rawStore, pipeState) {
  ...
  var _inputArgs = input.length > 0 ? input.map(function (key) {
    return rawStore[key];
  }) : args;
  var injectedFn;
  ...
  if ('input' === fnName || 'output' === fnName) pipeState.result = fn.call(0, args, store, rawStore);else if (injectedFn) pipeState.result = executeInjectedFunc(_inputArgs, injectedFn, pipeState);else pipeState.result = fn.apply(0, _inputArgs);
