...
function executePipe(err, args, store, rawStore, pipeState) {
  var fn = pipeState.fn;
  ...
  if (output && 0 < output.length) {
    ...
    modifiedSet = function modifiedSet(key, value) {
    ...
    if (input.indexOf('set') > -1) {
      input.forEach(function (key, idx) {
        if ('set' === key) _inputArgs[idx] = modifiedSet;
      });
    }
  }

  if (_builder.FN_INPUT === fnName || _builder.FN_OUTPUT === fnName) pipeState.result = fn.call(0, args, store, rawStore);else if (_builder.FN_ERROR === fnName) pipeState.result = fn(_inputArgs, rawStore);else if (injectedFn) pipeState.result = executeInjectedFunc(_inputArgs, injectedFn, pipeState);else pipeState.result = fn.apply(0, _inputArgs);

  pipeState.fnReturned = true;

  // Call setDep if a plain object was returned
  if ((0, _store.isPlainObject)(pipeState.result)) (modifiedSet || rawStore.set)(pipeState.result);
