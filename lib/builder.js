...
var FN_WAIT = exports.FN_WAIT = 'wait';
var FN_ERROR = exports.FN_ERROR = 'error';
var FN_INPUT = exports.FN_INPUT = 'input';
var FN_OUTPUT = exports.FN_OUTPUT = 'output';
var FN_THROTTLE = exports.FN_THROTTLE = 'throttle';
 ...
 */
function createPipe(fn, input, output) {
  var fnType = typeof fn === 'undefined' ? 'undefined' : _typeof(fn);
...
function inputIsRequired(input, fnType) {
  if (!input && 0 !== input) throw new Error('\'input\' is required for ' + fnType + ' pipe.');
}

function onlyOneArgumentIsAllowed(args, name) {
  if (args.length > 1) throw new Error('Only 1 argument is allowed for function "' + name + '".');
}
  ...
  return {
    fn: function inputPipe(args, store, rawStore) {
      input.forEach(function (item, idx) {
        if (item) rawStore[item] = args[idx];
      });
      return true;
    },
    fnName: FN_INPUT,
...
function createOutputPipe(input) {
  onlyOneArgumentIsAllowed(arguments);
  var output = normalizeStringInput(input);
  var outputMap = {};
...
function createErrorPipe(errorFn, input) {
  ...
  return {
    fn: function errorPipe(args, rawStore) {
      if ('string' === typeof errorFn) errorFn = rawStore[errorFn];
      if ('function' !== typeof errorFn) throw new Error('The error handling function "' + errorFn + '" is not a function.');
...
function normalizeInput(input) {
  ...
  if ('string' === inputType || 'number' === inputType || null === input) input = [input];

  if (input && !Array.isArray(input)) {
    if ('object' === inputType) input = [input];else throw new Error('`input` should be either string, array or object of dependency maps when presents');
  }
...
function normalizeOutput(output) {
  if ('string' === typeof output) output = [output];

  if (output && !Array.isArray(output)) throw new Error('`output` should be either string or array of output names when presents');
